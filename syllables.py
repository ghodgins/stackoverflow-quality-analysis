...
Based on the algorithm in Greg Fast's perl module Lingua::EN::Syllable.
    http://cpansearch.perl.org/src/GREGFAST/Lingua-EN-Syllable-0.251/Syllable.pm

Implementation from /nltk_contrib/readability/syllables_en.py (modified)
...
Fails (by one syllable) for about 10-15 percent of /usr/dict/words.

The only way to get a 100 percent accurate count is to do a dictionary
lookup, so this is a small and fast alternative where more-or-less accurate
results will suffice, such as estimating the reading level of a document.
...
often counted as the number of digits.  Could convert "1998" to
"nineteen eighty eight" (or "one thousand nine hundred eighty
eight", or...), but this is not implemented.
