...
if __name__ == '__main__':
    ...
    with open('data/answers-parents-verybad.csv') as verybad_file:
        verybad = islice(csv.DictReader(verybad_file), N)
        verybad_head = [
            {
                'body': row['body'],
                'parent_title': row['parenttitle'],
                'parent_body': row['parentbody']
            }
            for row in verybad
        ]

    with open('data/answers-parents-bad.csv') as bad_file:
    ...
    with open('data/answers-parents-good.csv') as good_file:
    ...
    with open('data/answers-parents-verygood.csv') as verygood_file:
    ...
    plt.savefig('graphs/answers_confusion_matrix.png', dpi=100, transparent=True, bbox_inches='tight')
    ...
    plt.savefig('graphs/answers_confusion_matrix_normalized.png', dpi=100, transparent=True, bbox_inches='tight')
    ...
    plt.barh(range(len(top_importances)), top_importances, color='#539DCC', ecolor='r', align="center")
    plt.yticks(range(len(top_importances)), top_names)
    plt.ylim([-1, len(top_importances)])
    plt.gca().invert_yaxis()
    plt.savefig('graphs/answers_feature_importances.png', dpi=100, transparent=True)
